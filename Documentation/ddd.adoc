...
| `content_type` | TEXT | NOT NULL, DEFAULT "book" | Content type: "book" or "magazine". Determines organizational structure and TOC generation method.
| `isbn` | TEXT | | ISBN (International Standard Book Number) for book cartridges. Optional, validated for format.
| `series_name` | TEXT | | Series name for grouping related cartridges (e.g., "Lensman Series").
| `edition_name` | TEXT | | Edition name for grouping variant cartridges (e.g., "D&D 5th Edition").
| `series_order` | INTEGER | | Order within series for proper sequencing.
|===
...
| `chapter_title` | TEXT | | Title of the chapter/section this page belongs to. Used for book cartridges.
| `article_id` | TEXT | | Foreign key to `Articles.article_id`. Used for magazine cartridges to associate pages with articles. NULL for book cartridges.
...
| `issue_id` | TEXT | PRIMARY KEY | Unique identifier for the issue (e.g., "2025-01" or UUID).
| `issue_number` | TEXT | NOT NULL | Issue number or identifier (e.g., "Volume 1, Issue 1" or "January 2025").
| `publication_date` | TEXT | NOT NULL | Publication date in ISO 8601 format (YYYY-MM-DD) for chronological ordering.
| `issue_title` | TEXT | | Optional issue title or theme.
| `issn` | TEXT | | ISSN (International Standard Serial Number) for the magazine. Optional, validated for format.
| `cover_image_path` | TEXT | | Optional reference to issue-specific cover image (resource_id in Resources table).
| `volume_number` | INTEGER | | Optional volume number for multi-volume magazines.
| `metadata_json` | TEXT | | Optional JSON object containing additional issue metadata.
|===

**Purpose:** Stores issue metadata for magazine cartridges. Issues are organized chronologically by publication date and grouped by publication year in the TOC.
...
| `article_id` | TEXT | PRIMARY KEY | Unique identifier for the article (UUID v4 recommended).
| `issue_id` | TEXT | NOT NULL | Foreign key to `Issues.issue_id`. Links article to its issue.
| `article_title` | TEXT | NOT NULL | Title of the article.
| `author` | TEXT | | Author name(s) for the article. May be comma-separated for multiple authors.
| `department` | TEXT | | Department or category (e.g., "Technology", "Politics", "Arts").
| `article_order` | INTEGER | NOT NULL | Order of the article within its issue. Determines display order.
| `abstract` | TEXT | | Optional article abstract or summary.
| `keywords_json` | TEXT | | Optional JSON array of keywords or tags for the article.
| `metadata_json` | TEXT | | Optional JSON object containing additional article metadata.
|===

**Purpose:** Stores article metadata for magazine cartridges. Articles are organized within issues and ordered by `article_order`. Pages are linked to articles via `Content_Pages.article_id`.
...
| `target_type` | TEXT | NOT NULL | Type of navigation target: `page_id`, `anchor_id`, `page_and_anchor`, `custom_handler`, `issue_id`, `article_id`.
| `target_value` | TEXT | NOT NULL | Navigation target value:
  ...
  * For `issue_id`: The issue_id to navigate to (magazine cartridges)
  * For `article_id`: The article_id to navigate to (magazine cartridges)
...
The TOC generation method depends on the `content_type` field in the `Metadata` table:

**For Book Cartridges (`content_type = "book"`):**
...
**For Magazine Cartridges (`content_type = "magazine"`):**

. **Primary Structure (Publication Years):**
   a. Query `SELECT DISTINCT strftime('%Y', publication_date) as year FROM Issues ORDER BY year DESC`
   b. For each year, create TOC entry: `{ "type": "year", "title": year, "children": [...] }`

. **Secondary Structure (Issues within Year):**
   a. For each year, query `SELECT issue_id, issue_number, issue_title, publication_date FROM Issues WHERE strftime('%Y', publication_date) = ? ORDER BY publication_date ASC`
   b. For each issue, create TOC entry: `{ "type": "issue", "title": issue_number, "issue_id": issue_id, "children": [...] }`

. **Tertiary Structure (Articles within Issue):**
   a. For each issue, query `SELECT article_id, article_title, article_order FROM Articles WHERE issue_id = ? ORDER BY article_order ASC`
   b. For each article, query pages: `SELECT page_id, page_order FROM Content_Pages WHERE article_id = ? ORDER BY page_order`
   c. Create TOC entry: `{ "type": "article", "title": article_title, "article_id": article_id, "page_id": first_page_id, "children": [...] }`

. **Quaternary Structure (Pages within Article):**
   a. For each article, list pages ordered by `page_order`
   b. Parse HTML content for heading elements to create nested structure (optional, same as book headings)
...
== Content Rendering Architecture Implementation

**Status:** ✅ **MIGRATED** (as of 2025-12-27)

**Note:** This section has been updated to reflect the migration from Qt WebEngine to QTextDocument + Qt Quick/QML architecture. See `content-rendering-decision-analysis.adoc` and `2025-12-26_content_rendering_architecture_decision.adoc` for the decision rationale.

This section specifies the detailed implementation of content rendering using QTextDocument/QTextBrowser for HTML content and Qt Quick/QML for embedded applications in the Reader and Creator Tool applications.
...
* **Required Modules:**
  ...
  * `Qt6::Widgets` - QTextBrowser, QTextEdit, form widgets
  * `Qt6::Quick` - QML engine
  * `Qt6::QuickWidgets` - QQuickWidget for embedding QML
  * `Qt6::Qml` - QML type registration
  * `Qt6::Network` - Network access (if needed)

**Note:** Qt WebEngine module is **NOT** required and has been removed from the build.

=== HTML/CSS Standards Support

**HTML Support:**

* **HTML4 Subset:** QTextDocument supports HTML4 subset
* **Supported Elements:** Basic HTML elements (headings, paragraphs, lists, links, images, tables, etc.)
* **Not Supported:** HTML5 semantic elements, canvas, video, audio, Web Components

**CSS Support:**

* **CSS 2.1 Subset:** QTextDocument supports CSS 2.1 properties
* **Supported Properties:** Basic styling (colors, fonts, margins, padding, borders, text-align, etc.)
* **Not Supported:** CSS3 features (flexbox, grid, transforms, animations, transitions, CSS variables)
...
* Content must be HTML4 + CSS 2.1 compliant
...
* `QTextBrowser` instances use minimal memory (~5-10MB per instance)
* `QQuickWidget` instances use moderate memory (~20-30MB per QML app)
* Qt Widgets forms use minimal memory (~1-2MB per form)
* **Total:** Much lower memory footprint than WebEngine (no Chromium process)

**View Lifecycle:**

* Create views on-demand (lazy loading)
* Destroy views when cartridge is closed
* QML apps are isolated per instance (no shared state)

**Memory Monitoring:**

* Monitor total memory usage across all windows
* Display warning if total memory usage exceeds threshold (e.g., 200MB)
...
* QTextBrowser renders HTML synchronously (no async loading delays)
* Theme changes are instant (no flash) - palette-based styling
* QML apps load asynchronously but are isolated from content rendering

**Optimization:**

* Content is rendered once (no re-rendering on theme change)
* QML apps are cached in memory after first load
* Form widgets are lightweight and fast to render
