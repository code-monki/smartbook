...
==== Library Manager UI
[X] Implement `LibraryManager` class
  [X] Library view widget
  [X] Dual view support (Bookshelf/List View)
  [X] View toggle functionality
  [X] Cartridge display (title, author, year, cover)
  [X] Context menu (Open, Delete, etc.)
  [X] Library refresh
[X] Implement Bookshelf View
  [X] Grid layout with cover images
  [X] Cover image display
  [X] Title overlay
  [X] Click to open
[X] Implement List View
  [X] Table with columns (Title, Author, Edition/Version, Year)
  [X] Data sourced from manifest
  [X] Sortable columns
  [X] Context menu
[X] Implement performance optimization
  [X] Lazy loading of cover images
  [X] Manifest-based metadata (no file parsing)
  [X] 500ms load time target (100+ cartridges)
...
==== Reader View Window
[X] Implement `ReaderViewWindow` class
  [X] Qt WebEngineView integration
  [X] Content loading from cartridge
  [X] Multi-window support
  [X] Window state persistence
  [X] Window state restoration
  [X] Window minimize/restore
...
[X] Implement WebChannel bridge
  [X] `SmartbookBridge` JavaScript API
  [X] `requestAppConsent()` method
  [X] Form data persistence API
  [X] Sandbox file API
  [X] Error handling
...
[X] Implement settings reading
  [X] Read `Settings` table from cartridge
  [X] Read `Local_User_Settings` for overrides
  [X] Apply settings priority (User override > Author default > App default)
  [X] Apply to content rendering
[X] Implement settings persistence
  [X] Store user preference overrides
  [X] Scoped per cartridge (`cartridge_guid`)
...
==== Form Builder
[x] Implement `FormBuilder` class
  [x] Visual form builder interface
  [x] Form field creation (text, number, select, etc.)
  [x] Validation rule configuration
  [x] Form schema validation (JSON Schema)
  [x] Form definition storage
[x] Implement form integration
  [x] Form marker insertion in content
  [x] Form preview
  [x] Form definition JSON generation
...
[x] Implement `MetadataEditor` class
  [x] Metadata field editing (title, author, publisher, etc.)
  [x] Cover image import and management
  [x] GUID generation (UUID v4)
  [x] Schema version management
[x] Implement resource management
  [x] Asset import (images, files)
  [x] Resource storage in `Resources` table
  [x] Resource reference management
...
[X] Implement `CartridgeExporter` class
  [X] New cartridge creation (SQLite file)
  [X] Schema initialization
  [X] Content packaging
  [X] Metadata storage
  [X] Resource packaging
[X] Implement cartridge signing
  [X] H1 hash calculation (content hash - SHA-256)
  [X] Certificate selection (via CertificateManager)
  [X] Signature creation (L1/L2/L3) with OpenSSL EVP
  [X] Security data storage
[X] Implement export validation
  [X] Content validation
  [X] Schema validation
  [X] Required metadata validation
...
[X] Implement certificate management UI
[X] Implement CertificateManager backend
[X] Implement CertificateManagerDialog UI
[X] Integrate certificate selection into export workflow
  [X] Certificate import
  [X] Self-signed certificate generation
  [X] Certificate selection interface
  [X] Certificate storage
[X] Implement certificate operations
...
**IMPORTANT:** All tests MUST be written BEFORE implementation (TDD). See `tdd-practices.adoc` for TDD standards.

[ ] Security verification tests
  [X] T-SEC-01: L1 Commercial Trust
  [X] T-SEC-02: L2/L3 Initial Consent
  [X] T-SEC-03: Persistent Trust
  [X] T-SEC-04: Tampering Detection
  [X] T-SEC-05: Trust Revocation
[ ] Persistence tests
  [X] T-PERS-01: Manifest Creation
  [X] T-PERS-02: Multi-Window Isolation
  [X] T-PERS-03: Atomic Deletion
  [X] T-PERS-01: Import and Manifest Creation (test_importmanager)
  [X] T-PERS-01: Metadata Extraction (test_metadataextractor)
[X] UI tests
  [X] T-UI-01: Library Performance (test_librarymanager_performance.cpp - 3 tests passing)
  [X] T-UI-02: Dual View (test_libraryview_dualview.cpp - 3 tests passing)
[X] Creator Tool tests
  [X] T-CT-01 through T-CT-05: Content Editor (test_contenteditor.cpp - 7 tests passing)
  [X] T-CT-27 through T-CT-30: Form Builder (test_formbuilder.cpp - 6 tests passing)
  [X] T-CT-10 through T-CT-21: Cartridge Exporter (test_cartridgeexporter.cpp - 13 tests passing)
  [X] T-CT-22 through T-CT-26: Metadata Editor (test_metadataeditor.cpp - 7 tests passing)
  [X] Page Manager tests (test_pagemanager.cpp - 6 tests passing)
  [X] Integration tests (test_contenteditor_pagemanager_integration.cpp, test_contenteditor_form_integration.cpp)

**Reference:** `test-plan.adoc`

==== Integration Testing
[X] End-to-end import workflow
[X] End-to-end cartridge creation and export (core functionality complete)
[X] Security verification workflow
[X] Multi-window isolation
...
[X] Invalid cartridge format
[X] Database connection errors
[X] Signature verification errors
[X] Import errors
...
* Qt 6.x (Core, Widgets, Quick, QuickWidgets, Network)
* **Note:** WebEngine dependency removed - using QTextDocument + QML instead
...
== Content Rendering Architecture Migration

**Status:** ⚠️ **PENDING MIGRATION**

**Decision:** Migrate from Qt WebEngine to QTextDocument + Qt Quick/QML architecture.

**Rationale:** See `content-rendering-decision-analysis.adoc` and `2025-12-26_content_rendering_architecture_decision.adoc`.

**Implementation Plan:** See `content-rendering-implementation-plan.adoc`.

**Impact:**
* Reader View: Migrate from `QWebEngineView` to `QTextBrowser`
* Content Editor: Migrate from `QWebEngineView` to `QTextEdit`
* Embedded Apps: Migrate from JavaScript SPAs to QML applications
* Forms: Migrate from JavaScript forms to Qt Widgets
* WebChannel Bridge: Replace with QML property binding

**Note:** Current Phase 1 implementation uses WebEngine. Migration is required before Phase 1 completion.
