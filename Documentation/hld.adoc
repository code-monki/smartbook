...
// Note: The verification algorithm diagram is defined in diagrams.adoc.

=== Trust Policy Determination

. **Identity:** The `SignatureVerifier` reads the `cartridge_guid` and security data (H1) from the cartridge.
. **Integrity Check:** The verifier recalculates H2 (content hash) and compares H1 vs. H2 (Tampering Detection, FR-2.4.4).
. **Local Trust Check:** The verifier queries the **Singleton `LocalDBManager`** to check the `Local_Trust_Registry` for a user-granted `PERSISTENT` policy.
. **Final Policy:** A final **Effective Trust Policy** is determined: `WHITELISTED`, `CONSENT_REQUIRED`, or `REJECTED`.
...
. **Load Navigation Structure:** Reader queries `Navigation_Structure` table on cartridge open
. **Render Navigation Bar:** HTML navigation bar generated from structure data
. **User Interaction:** User selects menu item from dropdown
. **Target Resolution:** Reader resolves navigation target (page_id, anchor, custom handler)
. **Navigation:** Reader navigates to target and updates reading position/history
...
. On library load or cartridge import, Library Manager queries `Local_Library_Manifest` for unique `series_name` and `edition_name` values
. For each unique value, creates or updates group in `Local_Cartridge_Groups` (group_type: "series" or "edition")
. Adds cartridges to group memberships based on metadata
. Auto-grouped series/editions are read-only (managed by system)

**Custom Collections:**

. User creates custom collection via Library Manager UI
. Collection stored in `Local_Cartridge_Groups` (group_type: "custom_collection")
. User adds/removes cartridges to/from collection
. Custom collections are user-managed and local-only

**Grouping Display:**

. Library Manager provides filter/view options: "All", "By Series", "By Edition", "Custom Collections"
. When filter selected, Library Manager queries appropriate groups and displays members
. Series members ordered by `series_order` (if available), otherwise alphabetical
. Edition and custom collection members ordered alphabetically (or by user-defined `display_order`)
