...
**Process:**
. Converts Mermaid diagrams to SVG (or PNG if SVG fails)
. Processes AsciiDoc files with Asciidoctor PDF
. Outputs PDFs to `build/docs/`
...
**Build Process:**
. Builds common library (shared library)
. Builds Reader application
. Builds Creator Tool application
. Builds Server application (Phase 2)
...
. **Mermaid → SVG:** Uses `mmdc` (Mermaid CLI) to convert `.mmd` files to SVG
. **SVG Fallback:** If SVG conversion fails, converts to PNG instead
. **Diagram Inclusion:** Diagrams are included inline in AsciiDoc documents
